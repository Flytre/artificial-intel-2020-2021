...
def player_move(word: str):
    ...
    for letter in dictionary.valid_letters(word, possibles):
        ...
        if attempt in dictionary.word_bank:
            continue
        result = ai_step_1(attempt, dictionary.retrieve_descendants(attempt, possibles))
        if result is True:
...
def ai_step_1(word: str, possibles: set):
    for letter in dictionary.valid_letters(word, possibles):
        attempt = word + letter
        if attempt in dictionary.word_bank:
            continue
        result = ai_step_2(attempt, dictionary.retrieve_descendants(attempt, possibles))
        if result is False:
            return result
    return True


def ai_step_2(word: str, possibles: set):
    for letter in dictionary.valid_letters(word, possibles):
        attempt = word + letter
        if attempt in dictionary.word_bank:
            continue
