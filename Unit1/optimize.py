...
class Puzzle:
    ...
    def get_taxicab(self):
        if self.taxicab_score == None:
            self.taxicab_score = self.taxicab()

        return self.taxicab_score
    ...
    def taxicab(self):
        if self.parent == None:
            ...
            self.taxicab_score = score
            return score

        else:
            # reconcile with parent
            for index, ch in enumerate(self.board):
                if self.board[index] == self.parent.board[index] or self.board[index] == '.':
                    continue
                else:
                    # get them
                    local_index = index # the one in common that isn't a "."
                    parent_index = self.parent.board.index(self.board[local_index])
                    goal_index = self.goal_dict[self.board[index]]

                    if self.taxi_dist_one_char(local_index,goal_index) < self.taxi_dist_one_char(parent_index,goal_index):
                        self.taxicab_score = self.parent.get_taxicab() - 1
                    else:
                        self.taxicab_score = self.parent.get_taxicab() + 1
            return self.taxicab_score

    def taxi_dist_one_char(self, index, goal_index):

        index_row, index_col = index % self.size, index // self.size
        goal_row, goal_col = goal_index % self.size, goal_index // self.size
        return abs(index_row - goal_row) + abs(index_col - goal_col)
...
def a_star(puzzle: Puzzle):
    closed = set()
    start_val = (puzzle.get_length() + puzzle.get_taxicab(), puzzle)
    fringe = [start_val]
        ...
        if item[1] not in closed:
            ...
            for child in children:
                if child not in closed:
                    heapq.heappush(fringe, (child.get_length() + child.get_taxicab(), child))
    return None
