...
def f(
        n): return 1 if n > 0 else 0  # This is the step function, defined with a single numeric input and a single numeric output


# print(f(x))  # If you uncomment this, it will throw an error!  The original function cannot accept a matrix as an argument
new_f = np.vectorize(
    f)  # This creates a function that applies the original function to each element of a matrix individually
print(new_f(x))
print()
